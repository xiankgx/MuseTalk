...
def parse_args():
    ...
    parser.add_argument(
        "--testing_speed",
        action="store_true",
        help="Whether to caculate the running time",
    )
    parser.add_argument(
        "--train_batch_size",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000,
        help=("Conduct validation every X updates."),
    )
    parser.add_argument(
        "--val_out_dir",
        type=str,
    ...
    # XXX Can't really switch this on the fly because we are pre-processing the data in advance extracting audio features using a certain model!
    parser.add_argument(
        "--whisper_model_type",
        type=str,
        default="tiny",
...
def main():
    ...
    if args.whisper_model_type == "tiny":
        ...
        # position_encoder = PositionalEncoding(d_model=1280)
        pe = PositionalEncoding(d_model=1280)
    else:
        raise Exception(f"Unsupported whisper_model_type: {args.whisper_model_type}")
    ...
    frames_paths = glob.glob(f"{args.data_root}/**/frames/*.jpg", recursive=True)
    train_dataset = HDTFDataset(frames_paths)
    print("train_dataset:", len(train_dataset))
    train_data_loader = data_utils.DataLoader(
        train_dataset, batch_size=args.train_batch_size, shuffle=True, num_workers=8
    )
    print("loading val_dataset ...")
    val_dataset = HDTFDataset(np.random.choice(frames_paths, 20))
        ...
        for step, (ref_image, image, _, mask, audio_feature) in enumerate(
            ...
            with accelerator.accumulate(unet):
                ...
                masked_latents = vae.encode(
                    masked_image.reshape(image.shape).to(
                        dtype=weight_dtype
                    )  # masked image
                ).latent_dist.sample()
