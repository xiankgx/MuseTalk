...
def load_all_model(
    whisper_checkpoint: str = "./models/whisper/tiny.pt",
    vae_model_id_or_path: str = "./models/sd-vae-ft-mse/",
    unet_config_path: str = "./models/musetalk/musetalk.json",
    unet_checkpoint: str = "./models/musetalk/pytorch_model.bin",
    audio_features_dim: int = 384,
):
    audio_processor = Audio2Feature(model_path=whisper_checkpoint)

    vae = VAE(model_path=vae_model_id_or_path)

    unet = UNet(unet_config=unet_config_path, model_path=unet_checkpoint)

    pe = PositionalEncoding(d_model=audio_features_dim)
