...
class VAE(nn.Module):
    ...
    def preprocess_img(
        self, img_name, mask=None, half_mask=False, enable_gx_mods: bool = False
    ):
        """
        ...
        # c, b, h, w -> c, h, w (since b=1)
        x = torch.squeeze(torch.FloatTensor(x))

        # XXX Original method: apply masking before normalization of images
        if not enable_gx_mods:
            if mask is not None:
                # h, w
        ...
        # XXX New method: apply masking after normalization of images
        if enable_gx_mods:
            if mask is not None:
                # h, w
                mask = mask / np.float32(255)
                ...
                x = x * self._mask_tensor

        x = x.unsqueeze(0)  # [1, 3, 256, 256] torch tensor
        x = x.to(self.vae.device)

        return x, mask.unsqueeze(
            0
        ) if mask is not None else self._mask_tensor.unsqueeze(0)
    ...
    def get_latents_for_unet(
        self, img, mask=None, ref_img=None, enable_gx_mods: bool = False
    ):
        """
        ...
        image_tensor, mask = self.preprocess_img(
            img, mask=mask, half_mask=False, enable_gx_mods=enable_gx_mods
        )  # [1, 3, 256, 256] RGB, torch tensor
        masked_latents = self.encode_latents(
            image_tensor
        )  # [1, 4, 32, 32], torch tensor

        ref_image_tensor, _ = self.preprocess_img(
            ref_img, half_mask=False, enable_gx_mods=enable_gx_mods
        )  # [1, 3, 256, 256] RGB, torch tensor
        ref_latents = self.encode_latents(
            ref_image_tensor
        )  # [1, 4, 32, 32], torch tensor

        assert masked_latents.shape == (1, 4, 32, 32)
        assert ref_latents.shape == (1, 4, 32, 32)

        if enable_gx_mods:
            if mask.shape[-2:] != masked_latents.shape[-2:]:
                mask = torch.nn.functional.interpolate(
                    mask, masked_latents.shape[-2:], mode="nearest"
                )
            ...
            latent_model_input = torch.cat(
                [
                    mask.to(device=self.vae.device, dtype=masked_latents.dtype),
                    masked_latents,
                    ref_latents,
                ],
                dim=1,
